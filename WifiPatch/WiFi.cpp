...
int WiFiClass::begin()
{
    ...
    // Auto Connect set, the CC31xx device tries to automatically reconnect to one of its stored profiles, each time the connection fails or the device is rebooted.
    if(sl_WlanPolicySet(SL_POLICY_CONNECTION,SL_CONNECTION_POLICY(1,0,0,0,0),NULL,0) < 0) return -1;

    //10 minutes scan interval for connection fail
    unsigned long intervalInSeconds = 600;
    #define SL_SCAN_ENABLE  10
    sl_WlanPolicySet(SL_POLICY_SCAN,SL_SCAN_ENABLE, (unsigned char *)&intervalInSeconds,sizeof(intervalInSeconds));
    return 0;
}

int WiFiClass::clearProfiles(){
    bool init_success = init();
    if (!init_success) {
        return WL_CONNECT_FAILED;
    }
    if(sl_WlanProfileDel(0xff) < 0) return -1;
    return 0;
}
...
int WiFiClass::begin(char* ssid)
{
    ...
    if(sl_WlanProfileDel(0xff) < 0) return WL_CONNECT_FAILED;
    //
